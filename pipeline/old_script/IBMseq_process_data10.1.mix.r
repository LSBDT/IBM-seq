...
process_graph_data_parallel <- function(x, read_path, save_path) {
  ...
  write_log <- function(message) {
    cat(message, file = "process_log.mix.txt", append = TRUE, sep = "\n") #名前
    }
  # データを処理する関数
  process_data <- function(name) {
    ...
    df_membership$sum <- rowSums(df_membership[, 2:13], na.rm = TRUE) #抗体の数で変わる。5種類x5種類なら2:13
...
process_dataset <- function(dataset_name) {
  ...
  membership_df_filter <- membership_df %>% filter(membership %in% df_membership_sum$membership[df_membership_sum$total > 1000])
  cluster_list <- split(membership_df_filter$name, membership_df_filter$membership) 
  suffixes <- c("UMI1" = "\\.t1$", "UMI2" = "\\.t2$", "UEI1" = "\\.e1$", "UEI2" = "\\.e2$")
...
# nohup Rscript process_data10.mix.R "V5P2_rB_mix_1_S6,V5P2_6aB_mix_1_S13,V5P2_24aB_mix_1_S20" "/user/med/nimura/BIKEN20240315" "/user/med/nimura/BIKEN20240315/V5P2_mix" &

# nohup Rscript process_data.mix.R "V5P2_rB_mix_2_S6,V5P2_6aB_mix_2_S13,V5P2_24aB_mix_2_S19" "/data/nimura_data/BIKEN20240401/F7713_d770_20240401163222" "/data/nimura_data/BIKEN20240401/F7713_d770_20240401163222/V5P2_mix" &
