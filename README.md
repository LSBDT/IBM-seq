...
IBMseqパイプライン実行後の出力スクリプトを使用した解析用のRスクリプトやDown samplingなどのシミュレーションのためのshellスクリプトです。

**主な機能:**

-  IBMseq解析結果のクラスター解析
  - cluster数・subgraph数の計算および閾値設定のための解析  
  - cycle rank / tree-likeness などのネットワーク指標の計算 
- シミュレーション
  - Down sampling 用データの作成
  - MIXネットワークから各抗体を除去したデータの生成

詳細は [data_analysis/README.md](data_analysis/README.md) をご覧ください。
